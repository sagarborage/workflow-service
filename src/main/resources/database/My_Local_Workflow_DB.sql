...
CREATE DATABASE IF NOT EXISTS `workflow` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */;
USE `workflow`;
...
CREATE TABLE IF NOT EXISTS `company` (
  ...
  `address_type_id` int(10) DEFAULT NULL,
  `is_active` tinyint(1) DEFAULT 1,
  PRIMARY KEY (`company_id`) USING BTREE,
  UNIQUE KEY `UUID` (`uuid`),
  KEY `fk_company_tenant` (`tenant_id`),
  KEY `fk_company_company_type` (`company_type_id`),
  KEY `fk_company_address` (`address_type_id`),
  CONSTRAINT `fk_company_address` FOREIGN KEY (`address_type_id`) REFERENCES `company_address` (`address_type_id`),
...
DROP TABLE IF EXISTS `confirm_through`;
...
DROP TABLE IF EXISTS `glass_specification`;
...
DROP TABLE IF EXISTS `glass_thickness`;
...
DROP TABLE IF EXISTS `pro_forma_invoice`;
...
DROP TABLE IF EXISTS `pro_forma_invoice_item`;
...
DROP TABLE IF EXISTS `service_rate_invoice`;
